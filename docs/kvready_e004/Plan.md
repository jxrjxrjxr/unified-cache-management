...
交给黄区的内容包括分支和提交、环境配置模板、固定测试入口、执行顺序、反馈样式与停止条件。首次环境信息由准备命令汇总，后续批次只需拉取指定提交、启动对应模式和运行测试。已有运行环境和依赖优先复用。
...
- **黄区接入。** 用户于 2026-09-23 回传官方 `feature_26h1@3f748a5`、原目录 `/home/j00977581/unified-cache-management` 和 Python `3.12.13`，构建缓存、原生库及 E001–E003 未提交内容仍在原位。蓝区核对该提交到 E004 为 19 个新增文件与两个请求统计 Python 文件修改，原生源码及构建脚本一致。接入采用保留原目录、沿用编译产物与既有环境的路线，官方 `origin` 保留，个人仓库使用 `personal`。下一步只读补齐已跟踪修改路径与解释器绝对路径，再确定具体保全、冲突核对和切换命令；详情见仓库 Runbook 附录 A、B。
...
- **部署待核实。** 现场模型路径、镜像、vLLM/Ascend/UCM 实际提交、ASU 是否供四个 worker 共享、GQA 逐层布局支持、现有 PD 服务和直传后端。GQA 为多个查询头共享较少 KV 头的注意力结构。准备命令统一采集这些事实，保持用户反馈简短。
