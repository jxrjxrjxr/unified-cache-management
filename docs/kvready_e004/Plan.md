...
- **黄区接入。** 用户于 2026-09-23 补充，黄区使用 2026 年 9 月初的官方 `feature_26h1`，保留未提交的编译产物和 E001–E003 实验内容。代码接入先核对实际提交、未跟踪与忽略项、解释器和原生库位置；官方 `origin` 保留，个人 E004 使用独立远端。优先保留原构建目录及路径，按实际差异决定直接复用旧库或必要的增量构建。运行说明以六条只读检查为第一步，迁移与安装命令在反馈后给出。
