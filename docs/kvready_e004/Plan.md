...
模型的 KV 布局决定搬运量。E004 选用已有的 Qwen3-4B，其标准逐层 GQA（多个查询头共享一组 KV 头）适配既有执行路径。按官方配置的 36 层、8 个 KV 头、每头 128 维，BF16（每个数值占 2 字节）的 KV 约为每 token 144 KiB，6144-token 前缀合计为 0.84375 GiB，字节数按两个 TP 分片合计。结合实际传输速率，即可比较前缀恢复时间与 P 的计算窗口。[官方配置](https://huggingface.co/Qwen/Qwen3-4B/blob/main/config.json)
...
交给黄区的内容包括分支和提交、新机器的目录与容器建立、编译命令、自动配置、服务就绪标志、固定测试包、通信演示和统一反馈格式。模型固定为已有 Qwen3-4B，ASU 参数取原脚本，容量按实际布局和内存条件估算；命令的预期输出与条件分支指导后续动作。详细配置与日志保留在黄区，正常流程完成后回传四行摘要，失败批次按运行说明结束并汇总。容器与编译方式沿用原运行手册。
...
| `test/kvready_e004/test_prepare.py` | 新机器配置、Qwen3 KV 布局与内存准入检查 |
...
| 项目 | 初版建议值与规则 |
|---|---|
| 模型 | 固定使用 `/home/models/Qwen3-4B`；模板指定 `enable_thinking=False`，让短输出直接回答问题 |
...
| 外部存储规划 | 新建供 E004 使用的 ASU inmem 服务；按真实布局核算 360 条完整上下文与 25% 余量，再加独立演示。准备脚本读取主机及容器内存余量并预留空间，在启动服务前检查；对象保留到固定包结束 |
...
测试数据使用真实语义文本和明确问题；精确 token ID 由同一分词器产生后重复使用，对话模板计入固定的 8192-token 输入。缓存准备直接采用相同前缀 token ID，实际块键作为核对依据。不同会话族保留自然的文档结构和独立前缀身份。以 Qwen3-4B、8192 输入 token、128-token 块计算，360 条完整上下文加 25% 余量为 506.25 GiB，通信演示另占 0.84375 GiB；运行脚本按本地模型配置重算。
...
- **黄区接入。** 2026-09-23 用户告知原服务器已回收，新机器仍为八卡 950DT，个人目录与容器尚未建立。Runbook 改为建立 `/home/j00977581`、直接克隆个人分支作为 `origin`、沿用原手册容器与编译方式、自动生成配置后执行固定包。GitCode 按原 `ucm-adapt-0910` 分支直接克隆编译；按用户要求免去其核验和回传。旧机器迁移流程保留于 Git 历史和实现日志。
...
- **现场配置与执行记录。** 新机器采用 `/home/models/Qwen3-4B`，模型模板关闭思考模式；`prepare.py` 从本地模型配置与原 ASU 参数生成配置文件，解释器取容器实际 Python。固定包加演示预算约 507.094 GiB；inmem 可用空间按主机可用内存与容器限额余量的较小值，预留至少 64 GiB 且不少于四分之一后估算。该值是准入快照，不是已分配配额或锁定内存。ASU 新进程供 E004 使用，配置、内存依据和实际版本保存在黄区，结果在批次结束后统一反馈。
...
- **通信资源口径。** 每卡 256 MiB 上限覆盖测试持有的数据和显式 HCCL 通信缓冲区；ASU、CANN（昇腾计算运行时）的内部工作区由现场软件栈管理，另行计入设备余量。演示复用一个层的数据缓冲区，在上一次读取完成后覆盖同一组专用存储对象；Qwen3-4B 的演示对象约占 0.84375 GiB。
- **测试边界。** 主流程、有限正确性检查和固定批次属于交付范围；部分层重算、任意前缀切点、异构 TP、跨机和大规模故障矩阵留在后续研究。模拟和本地接口检查分别标明，真实 NPU（神经网络处理器）结果由用户在黄区执行后记录。
- **依赖管理。** 蓝区复用现有 uv 管理的解释器和环境；黄区按原镜像与构建配方创建新环境，使用容器配套 Python 编译安装 UCM。E004 的专用测试沿用已有依赖；额外的依赖缺口或框架接口缺口以首个具体错误集中处理。
